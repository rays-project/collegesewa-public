...
const VideoAndCollegesPage = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (scrollTop + clientHeight >= scrollHeight - 10) setShowViewMore(true);
          ...
          <div className="col-lg-6 position-relative">
            ...
            >
              {filteredColleges.map((c) => (
                ...
                >
                  <div>
                    <h6 className="mb-1 fw-bold text-dark">{c.rank}. {c.name}</h6>
                    <small className="text-secondary">{c.mode} Mode</small>
                  </div>
                  <div className="d-flex gap-3 mt-2 mt-md-0 text-muted">
                    <span title="Rating"><FaStar className="text-warning" /> {c.rating}</span>
                    <span title="Students"><FaUsers /> {c.students}</span>
                    <span title="Fees"><FaRupeeSign /> {c.fees}</span>
                  </div>
                </div>
              ))}
            </div>
